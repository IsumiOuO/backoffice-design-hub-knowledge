...
## 發送訂單訊息訂單資訊 / Send Order Message Order Info
- canonicalId: `send-order-message-order-info`
- canonicalFamilyId: `send-order-message-order-info`（同一 family 的其他 RWD/Theme variants可用此值查找）
- kind: `component`
- sourceLayer: `candidate`
- status: `approved`
- AI 使用資格: `allowed`（allowed=可正式使用；reference-only=只供理解/參考；blocked=不得使用）
- aliases: —
- Figma: 03_共用模式 / Shared Patterns / [send-order-message-order-info] 發送訂單訊息訂單資訊 / Send Order Message Order Info / `155:347323` / https://www.figma.com/design/UX9EY190SGnpNiowHlwiUB?node-id=155-347323
- Figma 原生類型: `COMPONENT_SET`; Library Key=available; publishStatus=`NOT_CHECKED（安全匯出，以 Figma Library 面板為準）`
- Library Key: component=`—`; componentSet=`c010fe9ced743600d4e6a189cc54082a40946704`
...
- 功能說明: 呈現發送訂單訊息時的訂單資訊欄位，可顯示一般文字、特殊文字或時間內容。
- 遷移備註: 具有正式 variants，但缺少目前產品流程使用證據且功能邊界可能屬其他功能，先保留候選等待確認。
- Page Type: List
- Responsive Variants: unknown
- Variant Key: unknown|unknown|default
- States: default
- Components: send-order-message-order-info
- Variants: State / Size / Type / Layout / Theme / Density / Orientation / Emphasis
- Theme: explicit=unknown, strategy=manual, support=unknown
- Review: reviewed / confidence=0.74
- Original Review Action: —
- Evidence: migrationScore=50；usageEvidence 為 figma-main-component，但 directInstanceCount=0、nestedDependencyCount=0、usedByScreenCount=0、usedByComponentCount=0。；仍是正式 Component Set，具有文字、特殊文字、時間 3 個 variants；但目前掃描到的事件損益產品畫面沒有使用，名稱亦偏向其他訂單訊息流程。
- Legacy mapping: —
- Migration Score/Source: — / 報表紀錄 - 事件損益查詢 / 投分歷史紀錄 /         Key Screens 🔖v0.1.1-20260717(新增補扣單) / 10005:293286
- AI usage rules: 優先使用 internal-core，再 vendor-core，再 shared-pattern/feature；不得自行重建 Vendor Master。

## 事件損益操作彈窗 / Event Profit and Loss Action Dialog
- canonicalId: `event-profit-loss-action-dialog`
- canonicalFamilyId: `event-profit-loss-action-dialog`（同一 family 的其他 RWD/Theme variants可用此值查找）
- kind: `component`
- sourceLayer: `candidate`
- status: `approved`
- AI 使用資格: `allowed`（allowed=可正式使用；reference-only=只供理解/參考；blocked=不得使用）
- aliases: —
- Figma: 03_共用模式 / Shared Patterns / [event-profit-loss-action-dialog] 事件損益操作彈窗 / Event Profit and Loss Action Dialog / `155:347697` / https://www.figma.com/design/UX9EY190SGnpNiowHlwiUB?node-id=155-347697
- Figma 原生類型: `COMPONENT_SET`; Library Key=available; publishStatus=`NOT_CHECKED（安全匯出，以 Figma Library 面板為準）`
- Library Key: component=`—`; componentSet=`015241f25a85e32e195794c96c884896ee551c90`
- Component Properties: device (VARIANT: PC&Tablet/Mobile)（嚴格唯讀匯出僅由 Variant 名稱推導；TEXT／BOOLEAN／INSTANCE_SWAP 請由 MCP 即時讀取）
- 功能說明: 提供事件損益列表的操作對話框，讓營運人員選擇查看交易歷史紀錄或顯示事件詳細資訊，並支援 PC/Tablet 與 Mobile。
- 遷移備註: 目前沒有正式產品畫面引用，但具有完整跨裝置 variants 與明確事件操作流程，不能僅因使用量為 0 否定，先保留為候選。
- Page Type: List
- Responsive Variants: unknown
- Variant Key: unknown|unknown|default
- States: default
- Components: event-profit-loss-action-dialog
- Variants: State / Size / Type / Layout / Theme / Density / Orientation / Emphasis
- Theme: explicit=unknown, strategy=manual, support=unknown
- Review: reviewed / confidence=0.86
- Original Review Action: —
- Evidence: migrationScore=50；usageEvidence 為 figma-main-component，目前 directInstanceCount=0、nestedDependencyCount=0、usedByScreenCount=0、usedByComponentCount=0。；仍為正式 Component Set，具有 PC&Tablet、Mobile 2 個 device variants，childVariantCount=2。；textSummary 明確包含「操作」「交易歷史紀錄」「顯示詳細資訊」，並組合 head--dialog、Item--dialog、secondary button--dialog 與關閉控制，具有完整業務對話框結構。
- Legacy mapping: —
- Migration Score/Source: — / 報表紀錄 - 事件損益查詢 / 投分歷史紀錄 /         Key Screens 🔖v0.1.1-20260717(新增補扣單) / 10005:293820
- AI usage rules: 優先使用 internal-core，再 vendor-core，再 shared-pattern/feature；不得自行重建 Vendor Master。

## 事件損益提示與商戶選擇彈窗 / Event Profit and Loss Prompt and Merchant Selection Dialog
- canonicalId: `event-profit-loss-prompt-dialog`
- canonicalFamilyId: `event-profit-loss-prompt-dialog`（同一 family 的其他 RWD/Theme variants可用此值查找）
- kind: `component`
- sourceLayer: `candidate`
- status: `approved`
- AI 使用資格: `allowed`（allowed=可正式使用；reference-only=只供理解/參考；blocked=不得使用）
- aliases: —
- Figma: 03_共用模式 / Shared Patterns / [event-profit-loss-prompt-dialog] 事件損益提示與商戶選擇彈窗 / Event Profit and Loss Prompt and Merchant Selection Dialog / `155:347714` / https://www.figma.com/design/UX9EY190SGnpNiowHlwiUB?node-id=155-347714
- Figma 原生類型: `COMPONENT_SET`; Library Key=available; publishStatus=`NOT_CHECKED（安全匯出，以 Figma Library 面板為準）`
- Library Key: component=`—`; componentSet=`927ac6e1b52cb5ba184c74959531a5114220b709`
- Component Properties: Show Content#8060:6 (BOOLEAN)；device (VARIANT: PC&Tablet/Mobile)；狀態 (VARIANT: 請至少選擇一格資料欄位/選擇商戶)（嚴格唯讀匯出僅由 Variant 名稱推導；TEXT／BOOLEAN／INSTANCE_SWAP 請由 MCP 即時讀取）
- 功能說明: 處理事件損益查詢中的提示與商戶選擇情境，包括未選擇資料欄位的警示，以及在 PC/Tablet/Mobile 選擇商戶並確認。
- 遷移備註: 目前未接入正式產品 screen，但有明確的欄位選擇警示與商戶選擇流程，以及完整裝置 variants，因此保留候選等待確認。
- Page Type: List
- Responsive Variants: unknown
- Variant Key: unknown|unknown|default
- States: default
- Components: event-profit-loss-prompt-dialog
- Variants: State / Size / Type / Layout / Theme / Density / Orientation / Emphasis
- Theme: explicit=unknown, strategy=manual, support=unknown
- Review: reviewed / confidence=0.88
- Original Review Action: —
- Evidence: migrationScore=50；usageEvidence 為 figma-main-component，目前 directInstanceCount=0、nestedDependencyCount=0、usedByScreenCount=0、usedByComponentCount=0。；Component Set 有 4 個正式 variants：PC&Tablet/Mobile ×「請至少選擇一格資料欄位／選擇商戶」。；composition 包含 outcome--dialog、Alert/error_outline、商戶選擇元件、Checkbox、Scroll、head--dialog 與操作按鈕，顯示其不是單一裝飾元件而是完整流程彈窗。
- Legacy mapping: —
- Migration Score/Source: — / 報表紀錄 - 事件損益查詢 / 投分歷史紀錄 /         Key Screens 🔖v0.1.1-20260717(新增補扣單) / 10005:293898
- AI usage rules: 優先使用 internal-core，再 vendor-core，再 shared-pattern/feature；不得自行重建 Vendor Master。

## 事件損益自訂欄位顯示 / Event Profit and Loss Custom Columns Dialog
- canonicalId: `event-profit-loss-custom-columns-dialog`
- canonicalFamilyId: `event-profit-loss-custom-columns-dialog`（同一 family 的其他 RWD/Theme variants可用此值查找）
- kind: `component`
- sourceLayer: `candidate`
- status: `approved`
- AI 使用資格: `allowed`（allowed=可正式使用；reference-only=只供理解/參考；blocked=不得使用）
- aliases: —
- Figma: 03_共用模式 / Shared Patterns / [event-profit-loss-custom-columns-dialog] 事件損益自訂欄位顯示 / Event Profit and Loss Custom Columns Dialog / `155:347753` / https://www.figma.com/design/UX9EY190SGnpNiowHlwiUB?node-id=155-347753
- Figma 原生類型: `COMPONENT_SET`; Library Key=available; publishStatus=`NOT_CHECKED（安全匯出，以 Figma Library 面板為準）`
- Library Key: component=`—`; componentSet=`e96560854ad1e3488da7aab43059c938c9d553de`
- Component Properties: device (VARIANT: PC&Tablet/Mobile)（嚴格唯讀匯出僅由 Variant 名稱推導；TEXT／BOOLEAN／INSTANCE_SWAP 請由 MCP 即時讀取）
- 功能說明: 讓營運人員選擇事件損益列表要顯示的資料欄位，支援全部選擇、已選數量、捲動與 PC/Tablet、Mobile 排版。
- 遷移備註: 雖沒有正式產品引用，但欄位選取功能、跨裝置 variants 與完整 dialog 結構明確，先作為待接線候選保留。
- Page Type: List
- Responsive Variants: unknown
- Variant Key: unknown|unknown|default
- States: default
- Components: event-profit-loss-custom-columns-dialog
- Variants: State / Size / Type / Layout / Theme / Density / Orientation / Emphasis
- Theme: explicit=unknown, strategy=manual, support=unknown
- Review: reviewed / confidence=0.9
- Original Review Action: —
- Evidence: migrationScore=50；usageEvidence 為 figma-main-component，目前 directInstanceCount=0、nestedDependencyCount=0、usedByScreenCount=0、usedByComponentCount=0。；Component Set 有 PC&Tablet、Mobile 2 個 device variants，childVariantCount=2。；composition 含 30 個「項目」與 30 個 Checkbox instance，以及 Scroll、head--dialog、secondary button--dialog；textSummary 包含「自訂欄位顯示」「已選擇(5)」「全部選擇」。
- Legacy mapping: —
- Migration Score/Source: — / 報表紀錄 - 事件損益查詢 / 投分歷史紀錄 /         Key Screens 🔖v0.1.1-20260717(新增補扣單) / 10005:294228
- AI usage rules: 優先使用 internal-core，再 vendor-core，再 shared-pattern/feature；不得自行重建 Vendor Master。

## 投分歷史操作彈窗 / Bet History Action Dialog
- canonicalId: `bet-history-action-dialog`
- canonicalFamilyId: `bet-history-action-dialog`（同一 family 的其他 RWD/Theme variants可用此值查找）
- kind: `component`
- sourceLayer: `candidate`
- status: `approved`
- AI 使用資格: `allowed`（allowed=可正式使用；reference-only=只供理解/參考；blocked=不得使用）
- aliases: —
- Figma: 03_共用模式 / Shared Patterns / [bet-history-action-dialog] 投分歷史操作彈窗 / Bet History Action Dialog / `155:1163107` / https://www.figma.com/design/UX9EY190SGnpNiowHlwiUB?node-id=155-1163107
- Figma 原生類型: `COMPONENT_SET`; Library Key=available; publishStatus=`NOT_CHECKED（安全匯出，以 Figma Library 面板為準）`
- Library Key: component=`—`; componentSet=`a06312041f1be52035477e39516c23370fe4037c`
- Component Properties: device (VARIANT: PC&Tablet/Mobile)（嚴格唯讀匯出僅由 Variant 名稱推導；TEXT／BOOLEAN／INSTANCE_SWAP 請由 MCP 即時讀取）
- 功能說明: 提供投分歷史單筆操作對話框，讓營運人員選擇顯示詳細資訊或查看注單資訊。
- 遷移備註: usageEvidence 為 0，但 Component Set 具有 PC&Tablet、Mobile 2 個正式 variants，並包含「顯示詳細資訊」「注單資訊」的完整操作內容。
- Page Type: List
- Responsive Variants: unknown
- Variant Key: unknown|unknown|default
- States: default
- Components: bet-history-action-dialog
- Variants: State / Size / Type / Layout / Theme / Density / Orientation / Emphasis
- Theme: explicit=unknown, strategy=manual, support=unknown
- Review: reviewed / confidence=0.87
- Original Review Action: —
- Evidence: migrationScore=50；usageEvidence：directInstances=0、nestedDependencies=0、usedByScreenCount=0、usedByComponentCount=0。；Component Set childVariantCount=2；variants=['device=PC&Tablet', 'device=Mobile']。；usageEvidence 為 0，但 Component Set 具有 PC&Tablet、Mobile 2 個正式 variants，並包含「顯示詳細資訊」「注單資訊」的完整操作內容。
- Legacy mapping: —
- Migration Score/Source: — / 報表紀錄 - 事件損益查詢 / 投分歷史紀錄 /         Key Screens 🔖v0.1.1-20260717(新增補扣單) / 10038:97978
- AI usage rules: 優先使用 internal-core，再 vendor-core，再 shared-pattern/feature；不得自行重建 Vendor Master。

## 投分歷史自訂欄位顯示 / Bet History Custom Columns Dialog
- canonicalId: `bet-history-custom-columns-dialog`
- canonicalFamilyId: `bet-history-custom-columns-dialog`（同一 family 的其他 RWD/Theme variants可用此值查找）
- kind: `component`
- sourceLayer: `candidate`
- status: `approved`
- AI 使用資格: `allowed`（allowed=可正式使用；reference-only=只供理解/參考；blocked=不得使用）
- aliases: —
- Figma: 03_共用模式 / Shared Patterns / [bet-history-custom-columns-dialog] 投分歷史自訂欄位顯示 / Bet History Custom Columns Dialog / `155:1163124` / https://www.figma.com/design/UX9EY190SGnpNiowHlwiUB?node-id=155-1163124
- Figma 原生類型: `COMPONENT_SET`; Library Key=available; publishStatus=`NOT_CHECKED（安全匯出，以 Figma Library 面板為準）`
- Library Key: component=`—`; componentSet=`03b13ea97314c005e2db5fcee35e6d40c64fb230`
- Component Properties: device (VARIANT: PC&Tablet/Mobile)（嚴格唯讀匯出僅由 Variant 名稱推導；TEXT／BOOLEAN／INSTANCE_SWAP 請由 MCP 即時讀取）
- 功能說明: 讓營運人員選擇投分歷史列表要顯示的欄位，並支援 PC/Tablet 與 Mobile 排版。
- 遷移備註: usageEvidence 為 0，但 Component Set 有 PC&Tablet、Mobile 2 variants，並包含大量欄位項目、已選數量與全部選擇功能。
- Page Type: List
- Responsive Variants: unknown
- Variant Key: unknown|unknown|default
- States: default
- Components: bet-history-custom-columns-dialog
- Variants: State / Size / Type / Layout / Theme / Density / Orientation / Emphasis
- Theme: explicit=unknown, strategy=manual, support=unknown
- Review: reviewed / confidence=0.9
- Original Review Action: —
- Evidence: migrationScore=50；usageEvidence：directInstances=0、nestedDependencies=0、usedByScreenCount=0、usedByComponentCount=0。；Component Set childVariantCount=2；variants=['device=PC&Tablet', 'device=Mobile']。；usageEvidence 為 0，但 Component Set 有 PC&Tablet、Mobile 2 variants，並包含大量欄位項目、已選數量與全部選擇功能。
- Legacy mapping: —
- Migration Score/Source: — / 報表紀錄 - 事件損益查詢 / 投分歷史紀錄 /         Key Screens 🔖v0.1.1-20260717(新增補扣單) / 10029:52435
- AI usage rules: 優先使用 internal-core，再 vendor-core，再 shared-pattern/feature；不得自行重建 Vendor Master。

## 投分歷史提示與商戶選擇彈窗 / Bet History Prompt and Merchant Selection Dialog
- canonicalId: `bet-history-prompt-dialog`
- canonicalFamilyId: `bet-history-prompt-dialog`（同一 family 的其他 RWD/Theme variants可用此值查找）
- kind: `component`
- sourceLayer: `candidate`
- status: `approved`
- AI 使用資格: `allowed`（allowed=可正式使用；reference-only=只供理解/參考；blocked=不得使用）
- aliases: —
- Figma: 03_共用模式 / Shared Patterns / [bet-history-prompt-dialog] 投分歷史提示與商戶選擇彈窗 / Bet History Prompt and Merchant Selection Dialog / `155:1163318` / https://www.figma.com/design/UX9EY190SGnpNiowHlwiUB?node-id=155-1163318
- Figma 原生類型: `COMPONENT_SET`; Library Key=available; publishStatus=`NOT_CHECKED（安全匯出，以 Figma Library 面板為準）`
- Library Key: component=`—`; componentSet=`c76b740be316aa1bb3d32c2b715674a8cd02f3c1`
- Component Properties: Show Content#8060:6 (BOOLEAN)；device (VARIANT: PC&Tablet/Mobile)；狀態 (VARIANT: 請至少選擇一格資料欄位/選擇商戶)（嚴格唯讀匯出僅由 Variant 名稱推導；TEXT／BOOLEAN／INSTANCE_SWAP 請由 MCP 即時讀取）
- 功能說明: 處理投分歷史的欄位未選警示與商戶選擇情境，並支援桌面／平板與 Mobile。
- 遷移備註: usageEvidence 為 0，但 Component Set 具有 PC&Tablet/Mobile ×「請至少選擇一格資料欄位／選擇商戶」共 4 個正式 variants。
- Page Type: List
- Responsive Variants: unknown
- Variant Key: unknown|unknown|default
- States: default
- Components: bet-history-prompt-dialog
- Variants: State / Size / Type / Layout / Theme / Density / Orientation / Emphasis
- Theme: explicit=unknown, strategy=manual, support=unknown
- Review: reviewed / confidence=0.88
- Original Review Action: —
- Evidence: migrationScore=50；usageEvidence：directInstances=0、nestedDependencies=0、usedByScreenCount=0、usedByComponentCount=0。；Component Set childVariantCount=4；variants=['device=PC&Tablet, 狀態=請至少選擇一格資料欄位', 'device=PC&Tablet, 狀態=選擇商戶', 'device=Mobile, 狀態=請至少選擇一格資料欄位', 'device=Mobile, 狀態=選擇商戶']。；usageEvidence 為 0，但 Component Set 具有 PC&Tablet/Mobile ×「請至少選擇一格資料欄位／選擇商戶」共 4 個正式 variants。
- Legacy mapping: —
- Migration Score/Source: — / 報表紀錄 - 事件損益查詢 / 投分歷史紀錄 /         Key Screens 🔖v0.1.1-20260717(新增補扣單) / 10039:152121
- AI usage rules: 優先使用 internal-core，再 vendor-core，再 shared-pattern/feature；不得自行重建 Vendor Master。
...
## prediction-card/Variant4 / prediction-card Variant4
- canonicalId: `prediction-card-variant4`
- canonicalFamilyId: `prediction-card-variant4`（同一 family 的其他 RWD/Theme variants可用此值查找）
- kind: `component`
- sourceLayer: `local`
- status: `approved`
- AI 使用資格: `allowed`（allowed=可正式使用；reference-only=只供理解/參考；blocked=不得使用）
- aliases: —
- Figma: 03_共用模式 / Shared Patterns / [prediction-card-variant4] prediction-card/Variant4 / prediction-card Variant4 / `190:225085` / https://www.figma.com/design/UX9EY190SGnpNiowHlwiUB?node-id=190-225085
- Figma 原生類型: `COMPONENT`; Library Key=available; publishStatus=`NOT_CHECKED（安全匯出，以 Figma Library 面板為準）`
- Library Key: component=`a56795b19b83b76c8af11c3de3eeba31865e0cd7`; componentSet=`—`
- Component Properties: —（嚴格唯讀匯出僅由 Variant 名稱推導；TEXT／BOOLEAN／INSTANCE_SWAP 請由 MCP 即時讀取）
- 功能說明: 作為圖片預覽內容的 prediction card 尺寸／版型子元件，用於預覽畫布內呈現事件卡片。
- 遷移備註: 有大量預覽使用，但為單一 COMPONENT 子版型；需人工確認是否應併入 prediction-card Component Set 後再遷移
- Page Type: List
- Responsive Variants: unknown
- Variant Key: unknown|unknown|default
- States: default
- Components: prediction-card-variant4
- Variants: State / Size / Type / Layout / Theme / Density / Orientation / Emphasis
- Theme: explicit=unknown, strategy=variables, support=variables
- Review: reviewed / confidence=0.86
- Original Review Action: candidate
- Evidence: migrationScore=70；usageEvidence：directInstances=10、screens=34、components=1，method=figma-main-component；屬 preview 內部 prediction-card 子元件，並非 Component Set
- Legacy mapping: —
- Migration Score/Source: — / 資料庫管理 - 事件參數管理 / 參數類別管理 / 事件主類別管理 / 事件次類別管理 /         Key Screens 🔖v0.1.0-20260313-當前版本 / 6121:45352
- AI usage rules: 優先使用 internal-core，再 vendor-core，再 shared-pattern/feature；不得自行重建 Vendor Master。

## 表格操作項目 / Table Action Item
- canonicalId: `account-table-action-item`
- canonicalFamilyId: `account-table-action-item`（同一 family 的其他 RWD/Theme variants可用此值查找）
- kind: `component`
- sourceLayer: `candidate`
- status: `approved`
- AI 使用資格: `allowed`（allowed=可正式使用；reference-only=只供理解/參考；blocked=不得使用）
- aliases: —
- Figma: 03_共用模式 / Shared Patterns / [account-table-action-item] 表格操作項目 / Table Action Item / `190:225104` / https://www.figma.com/design/UX9EY190SGnpNiowHlwiUB?node-id=190-225104
- Figma 原生類型: `COMPONENT`; Library Key=available; publishStatus=`NOT_CHECKED（安全匯出，以 Figma Library 面板為準）`
- Library Key: component=`18b1aefae1b0df6cf5e3f71a961f8fae59e99ada`; componentSet=`—`
- Component Properties: —（嚴格唯讀匯出僅由 Variant 名稱推導；TEXT／BOOLEAN／INSTANCE_SWAP 請由 MCP 即時讀取）
- 功能說明: 提供帳號管理列表單一操作控制，供表格操作區重用。
- 遷移備註: 有跨畫面使用，但與同區域的操作 Component Set 功能重疊，需確認是否為獨立 canonical 元件。
- Page Type: List
- Responsive Variants: unknown
- Variant Key: unknown|unknown|default
- States: default
- Components: account-table-action-item
- Variants: State / Size / Type / Layout / Theme / Density / Orientation / Emphasis
- Theme: explicit=unknown, strategy=variables, support=variables
- Review: reviewed / confidence=0.84
- Original Review Action: candidate
- Evidence: usageEvidence: directInstanceCount=13, usedByScreenCount=7, effectiveUsageCount=13；usedByViewports=['desktop', 'tablet', 'mobile'], usedByThemes=['dark', 'light']；migrationScore=70; childVariantCount=0; usageCountMethod=figma-main-component；現有結構可重用，但使用覆蓋或 canonical 邊界仍需人工確認。
- Legacy mapping: —
- Migration Score/Source: — / 系統帳號 - 帳號管理列表 /         Key Screens 🔖v0.1.0-20260821 / 8048:16271
- AI usage rules: 優先使用 internal-core，再 vendor-core，再 shared-pattern/feature；不得自行重建 Vendor Master。

## 鍵值文字 / Key Value Text
- canonicalId: `key-value-text`
- canonicalFamilyId: `key-value-text`（同一 family 的其他 RWD/Theme variants可用此值查找）
- kind: `component`
- sourceLayer: `candidate`
- status: `approved`
- AI 使用資格: `allowed`（allowed=可正式使用；reference-only=只供理解/參考；blocked=不得使用）
- aliases: —
- Figma: 03_共用模式 / Shared Patterns / [key-value-text] 鍵值文字 / Key Value Text / `190:225118` / https://www.figma.com/design/UX9EY190SGnpNiowHlwiUB?node-id=190-225118
- Figma 原生類型: `COMPONENT_SET`; Library Key=available; publishStatus=`NOT_CHECKED（安全匯出，以 Figma Library 面板為準）`
- Library Key: component=`—`; componentSet=`a09055c0ceeb0a448a822cb9028d7edc09ed3b35`
- Component Properties: 內容#7480:12 (TEXT)；標題#7528:0 (TEXT)；Property 1 (VARIANT: Default)（嚴格唯讀匯出僅由 Variant 名稱推導；TEXT／BOOLEAN／INSTANCE_SWAP 請由 MCP 即時讀取）
- 功能說明: 以標題、分隔符與數值組合呈現簡單鍵值資訊。
- 遷移備註: 目前沒有直接產品畫面使用，但具有正式 Component Set／Variant 結構；保留為候選並需人工確認是否為現行流程元件。
- Page Type: List
- Responsive Variants: unknown
- Variant Key: unknown|unknown|default
- States: default
- Components: key-value-text
- Variants: State / Size / Type / Layout / Theme / Density / Orientation / Emphasis
- Theme: explicit=unknown, strategy=manual, support=unknown
- Review: reviewed / confidence=0.7
- Original Review Action: candidate
- Evidence: usageEvidence: directInstanceCount=0, usedByScreenCount=0, effectiveUsageCount=0；migrationScore=50; childVariantCount=1; usageCountMethod=figma-main-component；元件功能屬通用控制，inventory 未提供 existingMatch，需先與 Core Design System 比對。
- Legacy mapping: —
- Migration Score/Source: — / 系統帳號 - 帳號管理列表 /         Key Screens 🔖v0.1.0-20260821 / 10017:42904
- AI usage rules: 優先使用 internal-core，再 vendor-core，再 shared-pattern/feature；不得自行重建 Vendor Master。
...
## 次類別列表標題欄 / Subcategory List Title Header
- canonicalId: `subcategory-list-title-header`
- canonicalFamilyId: `subcategory-list-title-header`（同一 family 的其他 RWD/Theme variants可用此值查找）
- kind: `component`
- sourceLayer: `candidate`
- status: `approved`
- AI 使用資格: `allowed`（allowed=可正式使用；reference-only=只供理解/參考；blocked=不得使用）
- aliases: —
- Figma: 03_共用模式 / Shared Patterns / [subcategory-list-title-header] 次類別列表標題欄 / Subcategory List Title Header / `198:259892` / https://www.figma.com/design/UX9EY190SGnpNiowHlwiUB?node-id=198-259892
- Figma 原生類型: `COMPONENT_SET`; Library Key=available; publishStatus=`NOT_CHECKED（安全匯出，以 Figma Library 面板為準）`
- Library Key: component=`—`; componentSet=`80acc1f3e87de66ed2ff425b3ea2dbb30479f8f3`
- Component Properties: Property 1 (VARIANT: 名稱/Variant2)（嚴格唯讀匯出僅由 Variant 名稱推導；TEXT／BOOLEAN／INSTANCE_SWAP 請由 MCP 即時讀取）
- 功能說明: 提供查看次類別列表的標題欄內容，例如名稱欄與另一種標題配置，供對話框表頭組合使用。
- 遷移備註: 具有明確巢狀重用但尚未直接或間接出現在產品 screen；待確認查看次類別功能是否正式採用後再遷移。
- Page Type: Create
- Responsive Variants: unknown
- Variant Key: unknown|unknown|default
- States: default
- Components: subcategory-list-title-header
- Variants: State / Size / Type / Layout / Theme / Density / Orientation / Emphasis
- Theme: explicit=unknown, strategy=manual, support=unknown
- Review: reviewed / confidence=0.88
- Original Review Action: —
- Evidence: migrationScore=65，usageEvidence：directInstances=0、nestedDependencies=12、screens=0、components=4；被 6154:558626、6154:565614、6154:565662、6154:558686 等本地元件依賴，證明具有元件內部重用；Component Set 具「名稱／Variant2」2 個 variants
- Legacy mapping: —
- Migration Score/Source: — / 資料庫管理 - 事件參數管理 / 參數類別管理 / 事件主類別管理 / 事件次類別管理 /         Key Screens 🔖v0.1.1-20260423 / 6154:558682
- AI usage rules: 優先使用 internal-core，再 vendor-core，再 shared-pattern/feature；不得自行重建 Vendor Master。

## 次類別列表表頭 / Subcategory List Header
- canonicalId: `subcategory-list-header`
- canonicalFamilyId: `subcategory-list-header`（同一 family 的其他 RWD/Theme variants可用此值查找）
- kind: `component`
- sourceLayer: `candidate`
- status: `approved`
- AI 使用資格: `allowed`（allowed=可正式使用；reference-only=只供理解/參考；blocked=不得使用）
- aliases: —
- Figma: 03_共用模式 / Shared Patterns / [subcategory-list-header] 次類別列表表頭 / Subcategory List Header / `198:259901` / https://www.figma.com/design/UX9EY190SGnpNiowHlwiUB?node-id=198-259901
- Figma 原生類型: `COMPONENT_SET`; Library Key=available; publishStatus=`NOT_CHECKED（安全匯出，以 Figma Library 面板為準）`
- Library Key: component=`—`; componentSet=`e0d54422b5ede89596fb922cc8e67b0aa8f9b931`
- Component Properties: Property 1 (VARIANT: Default)（嚴格唯讀匯出僅由 Variant 名稱推導；TEXT／BOOLEAN／INSTANCE_SWAP 請由 MCP 即時讀取）
- 功能說明: 組合查看事件次類別列表的桌機／平板表頭，統一欄位標題與排序／操作區配置。
- 遷移備註: 已被對話框元件組合使用，但未有正式產品 screen 使用證據，因此先列為候選。
- Page Type: Create
- Responsive Variants: unknown
- Variant Key: unknown|unknown|default
- States: default
- Components: subcategory-list-header
- Variants: State / Size / Type / Layout / Theme / Density / Orientation / Emphasis
- Theme: explicit=unknown, strategy=manual, support=unknown
- Review: reviewed / confidence=0.86
- Original Review Action: —
- Evidence: migrationScore=65，usageEvidence：directInstances=0、nestedDependencies=4、screens=0、components=2；被 6154:565614 與 6154:558626 兩個本地元件依賴；Component Set 雖只有 Default variant，但 composition 內含 4 個 instance 並使用標題欄元件
- Legacy mapping: —
- Migration Score/Source: — / 資料庫管理 - 事件參數管理 / 參數類別管理 / 事件主類別管理 / 事件次類別管理 /         Key Screens 🔖v0.1.1-20260423 / 6154:558686
- AI usage rules: 優先使用 internal-core，再 vendor-core，再 shared-pattern/feature；不得自行重建 Vendor Master。
...
## 核取方塊 / Checkbox
- canonicalId: `checkbox`
- canonicalFamilyId: `checkbox`（同一 family 的其他 RWD/Theme variants可用此值查找）
- kind: `component`
- sourceLayer: `candidate`
- status: `approved`
- AI 使用資格: `allowed`（allowed=可正式使用；reference-only=只供理解/參考；blocked=不得使用）
- aliases: —
- Figma: 04_功能元件 / Feature Components / [checkbox] 核取方塊 / Checkbox / `190:229004` / https://www.figma.com/design/UX9EY190SGnpNiowHlwiUB?node-id=190-229004
- Figma 原生類型: `COMPONENT_SET`; Library Key=available; publishStatus=`NOT_CHECKED（安全匯出，以 Figma Library 面板為準）`
- Library Key: component=`—`; componentSet=`15842ae5a68dc6520ee39b09aeb9917788711072`
- Component Properties: 項目#10090:59 (TEXT)；狀態 (VARIANT: 已選取/尚未選取)（嚴格唯讀匯出僅由 Variant 名稱推導；TEXT／BOOLEAN／INSTANCE_SWAP 請由 MCP 即時讀取）
- 功能說明: 提供已選取與尚未選取的核取狀態，用於安全政策等多選設定。
- 遷移備註: 屬通用基礎控制，雖有高使用量，但應先比對 Core Design System，避免在 Hub 重複建立 Master。
- Page Type: List
- Responsive Variants: unknown
- Variant Key: unknown|unknown|default
- States: default
...
- Original Review Action: candidate
- Evidence: usageEvidence: directInstanceCount=144, usedByScreenCount=18, effectiveUsageCount=144；usedByViewports=['desktop', 'tablet', 'mobile'], usedByThemes=['dark', 'light']；migrationScore=105; childVariantCount=2; usageCountMethod=figma-main-component；元件功能屬通用控制，inventory 未提供 existingMatch，需先與 Core Design System 比對。
- Legacy mapping: —
- Migration Score/Source: — / 系統帳號 - 帳號管理列表 /         Key Screens 🔖v0.1.0-20260821 / 10090:123107
- AI usage rules: 優先使用 internal-core，再 vendor-core，再 shared-pattern/feature；不得自行重建 Vendor Master。
...
## 掛賣列表操作按鈕（PC，舊子集） / Sell List Action Button PC Legacy Subset
- canonicalId: `sell-list-action-button-pc-legacy-subset`
- canonicalFamilyId: `sell-list-action-button-pc-legacy-subset`（同一 family 的其他 RWD/Theme variants可用此值查找）
- kind: `component`
- sourceLayer: `candidate`
- status: `approved`
- AI 使用資格: `allowed`（allowed=可正式使用；reference-only=只供理解/參考；blocked=不得使用）
- aliases: —
- Figma: 04_功能元件 / Feature Components · 分頁 2 / [sell-list-action-button-pc-legacy-subset] 掛賣列表操作按鈕（PC，舊子集） / Sell List Action Button PC Legacy Subset / `144:4645` / https://www.figma.com/design/UX9EY190SGnpNiowHlwiUB?node-id=144-4645
- Figma 原生類型: `COMPONENT_SET`; Library Key=available; publishStatus=`NOT_CHECKED（安全匯出，以 Figma Library 面板為準）`
- Library Key: component=`—`; componentSet=`46f1da63cf74e51d31314c35c6f24e3a860a1974`
- Component Properties: type (VARIANT: 編輯/刪除/確認編輯/取消)；status (VARIANT: default/hover/disable)（嚴格唯讀匯出僅由 Variant 名稱推導；TEXT／BOOLEAN／INSTANCE_SWAP 請由 MCP 即時讀取）
- 功能說明: 較早或較精簡的桌面掛賣列表操作按鈕集合，提供編輯、刪除、確認編輯與取消等操作。
- 遷移備註: 與較完整的同名元件高度重疊，應先確認是否可合併／替換後再決定是否獨立遷移。
- Page Type: Other
- Responsive Variants: unknown
- Variant Key: unknown|unknown|default
- States: default
- Components: sell-list-action-button-pc-legacy-subset
- Variants: State / Size / Type / Layout / Theme / Density / Orientation / Emphasis
- Theme: explicit=unknown, strategy=manual, support=unknown
- Review: reviewed / confidence=0.88
- Original Review Action: —
- Evidence: usageEvidence：directInstances=96、usedByScreenCount=90、usedByComponentCount=6，仍有正式產品引用。；Component Set 只有 12 variants，類型為編輯、刪除、確認編輯、取消，缺少 7695:446803 的「最大值」variants。；與 7695:446803 同名但 Figma key 不同，結構與 variant 數亦不同，不能直接視為 exact duplicate。
- Legacy mapping: —
- Migration Score/Source: — / 事件設置 - 事件管理列表 / 新增事件&匯入議題 / 第三方事件審核列表 / 事件參數管理 / 單一事件結果 / 多時間軸事件 / 庫存管理 /         庫存管理🔖v0.1.1-20260731 / 7739:301985
- AI usage rules: 優先使用 internal-core，再 vendor-core，再 shared-pattern/feature；不得自行重建 Vendor Master。
...
## 次類別列表資料列 / Subcategory List Desktop Row
- canonicalId: `subcategory-list-desktop-row`
- canonicalFamilyId: `subcategory-list-desktop-row`（同一 family 的其他 RWD/Theme variants可用此值查找）
- kind: `component`
- sourceLayer: `candidate`
- status: `approved`
- AI 使用資格: `allowed`（allowed=可正式使用；reference-only=只供理解/參考；blocked=不得使用）
- aliases: —
- Figma: 04_功能元件 / Feature Components · 分頁 2 / [subcategory-list-desktop-row] 次類別列表資料列 / Subcategory List Desktop Row / `198:279505` / https://www.figma.com/design/UX9EY190SGnpNiowHlwiUB?node-id=198-279505
- Figma 原生類型: `COMPONENT_SET`; Library Key=available; publishStatus=`NOT_CHECKED（安全匯出，以 Figma Library 面板為準）`
- Library Key: component=`—`; componentSet=`9f0ce82aa3e71e7fd460e934f7990f3f3086f31b`
- Component Properties: Property 1 (VARIANT: 次類別 1/次類別 2/次類別 3)（嚴格唯讀匯出僅由 Variant 名稱推導；TEXT／BOOLEAN／INSTANCE_SWAP 請由 MCP 即時讀取）
- 功能說明: 呈現查看事件次類別列表中的單筆次類別名稱與操作內容，提供多筆範例 variants 供列表組合。
- 遷移備註: 具列表資料列 variants 與巢狀重用，但沒有正式產品畫面引用，先保留候選。
- Page Type: Create
- Responsive Variants: unknown
- Variant Key: unknown|unknown|default
- States: default
- Components: subcategory-list-desktop-row
- Variants: State / Size / Type / Layout / Theme / Density / Orientation / Emphasis
- Theme: explicit=unknown, strategy=manual, support=unknown
- Review: reviewed / confidence=0.88
- Original Review Action: —
- Evidence: migrationScore=65，usageEvidence：directInstances=0、nestedDependencies=12、screens=0、components=2；被 Light/Dark 查看次類別對話框元件依賴，具可驗證的巢狀重用；Component Set 含「次類別 1／次類別 2／次類別 3」3 個 variants
- Legacy mapping: —
- Migration Score/Source: — / 資料庫管理 - 事件參數管理 / 參數類別管理 / 事件主類別管理 / 事件次類別管理 /         Key Screens 🔖v0.1.1-20260423 / 6154:558690
- AI usage rules: 優先使用 internal-core，再 vendor-core，再 shared-pattern/feature；不得自行重建 Vendor Master。

## 掛載次類別 Active 清單 / Subcategory Selection Active List
- canonicalId: `subcategory-selection-active-list`
- canonicalFamilyId: `subcategory-selection-active-list`（同一 family 的其他 RWD/Theme variants可用此值查找）
- kind: `component`
- sourceLayer: `candidate`
- status: `approved`
- AI 使用資格: `allowed`（allowed=可正式使用；reference-only=只供理解/參考；blocked=不得使用）
- aliases: —
- Figma: 04_功能元件 / Feature Components · 分頁 2 / [subcategory-selection-active-list] 掛載次類別 Active 清單 / Subcategory Selection Active List / `198:279476` / https://www.figma.com/design/UX9EY190SGnpNiowHlwiUB?node-id=198-279476
- Figma 原生類型: `COMPONENT`; Library Key=available; publishStatus=`NOT_CHECKED（安全匯出，以 Figma Library 面板為準）`
- Library Key: component=`d472dd97e66075b5a1af2304da75c869e704be13`; componentSet=`—`
- Component Properties: Show Scroll#4293:31 (BOOLEAN)；Show Item 1#4293:33 (BOOLEAN)；Show Item 2#4293:35 (BOOLEAN)；Show Item 3#4293:37 (BOOLEAN)；Show Item 4#4293:39 (BOOLEAN)；Show Item 5#4293:41 (BOOLEAN)；Show Item 6#4293:43 (BOOLEAN)；Show Item 7#4293:45 (BOOLEAN)；Show Item 8#4293:47 (BOOLEAN)；Show Item 9#4293:49 (BOOLEAN)；Show Item 10#4293:51 (BOOLEAN)；Show Head#7120:1 (BOOLEAN)；Show Footer#7120:14 (BOOLEAN)（嚴格唯讀匯出僅由 Variant 名稱推導；TEXT／BOOLEAN／INSTANCE_SWAP 請由 MCP 即時讀取）
- 功能說明: 呈現掛載次類別清單的 Active 狀態，包含已選數量、次類別項目與清單控制。
- 遷移備註: 有少量正式使用與巢狀依賴，但只代表單一 Active 狀態；是否應併回 List Group canonical 需人工確認。
- Page Type: Create
- Responsive Variants: unknown
- Variant Key: unknown|unknown|default
- States: default
- Components: subcategory-selection-active-list
- Variants: State / Size / Type / Layout / Theme / Density / Orientation / Emphasis
- Theme: explicit=unknown, strategy=variables, support=variables
- Review: reviewed / confidence=0.8
- Original Review Action: —
- Evidence: inventory recommendation=candidate、migrationScore=30，usageCountMethod=figma-main-component；usageEvidence：directInstances=1、nestedDependencies=8、screens=1、components=2、viewport=desktop、theme=dark；此節點是單一 COMPONENT 而非完整 Component Set，名稱明確為 List Group/List/Active
- Legacy mapping: —
- Migration Score/Source: — / 資料庫管理 - 事件參數管理 / 參數類別管理 / 事件主類別管理 / 事件次類別管理 /         Key Screens 🔖v0.1.1-20260423 / 6082:27690
- AI usage rules: 優先使用 internal-core，再 vendor-core，再 shared-pattern/feature；不得自行重建 Vendor Master。
...
## prediction-card_1600/Variant4 / prediction-card_1600 Variant4
- canonicalId: `prediction-card-1600-variant4`
- canonicalFamilyId: `prediction-card-1600-variant4`（同一 family 的其他 RWD/Theme variants可用此值查找）
- kind: `component`
- sourceLayer: `local`
- status: `approved`
- AI 使用資格: `allowed`（allowed=可正式使用；reference-only=只供理解/參考；blocked=不得使用）
- aliases: —
- Figma: 04_功能元件 / Feature Components · 分頁 3 / [prediction-card-1600-variant4] prediction-card_1600/Variant4 / prediction-card_1600 Variant4 / `190:237464` / https://www.figma.com/design/UX9EY190SGnpNiowHlwiUB?node-id=190-237464
- Figma 原生類型: `COMPONENT`; Library Key=available; publishStatus=`NOT_CHECKED（安全匯出，以 Figma Library 面板為準）`
- Library Key: component=`be87fc55cc877750193e6674167b8f1393547842`; componentSet=`—`
- Component Properties: —（嚴格唯讀匯出僅由 Variant 名稱推導；TEXT／BOOLEAN／INSTANCE_SWAP 請由 MCP 即時讀取）
- 功能說明: 作為圖片預覽內容的 prediction card 尺寸／版型子元件，用於預覽畫布內呈現事件卡片。
- 遷移備註: 有大量預覽使用，但為單一 COMPONENT 子版型；需人工確認是否應併入 prediction-card Component Set 後再遷移
- Page Type: List
...
- Variant Key: unknown|unknown|default
- States: default
- Components: prediction-card-1600-variant4
- Variants: State / Size / Type / Layout / Theme / Density / Orientation / Emphasis
- Theme: explicit=unknown, strategy=variables, support=variables
- Review: reviewed / confidence=0.86
- Original Review Action: candidate
- Evidence: migrationScore=70；usageEvidence：directInstances=30、screens=32、components=3，method=figma-main-component；屬 preview 內部 prediction-card 子元件，並非 Component Set
- Legacy mapping: —
- Migration Score/Source: — / 資料庫管理 - 事件參數管理 / 參數類別管理 / 事件主類別管理 / 事件次類別管理 /         Key Screens 🔖v0.1.0-20260313-當前版本 / 6121:44812
- AI usage rules: 優先使用 internal-core，再 vendor-core，再 shared-pattern/feature；不得自行重建 Vendor Master。

## prediction-card_768/Variant2 / prediction-card_768 Variant2
- canonicalId: `prediction-card-768-variant2`
- canonicalFamilyId: `prediction-card-768-variant2`（同一 family 的其他 RWD/Theme variants可用此值查找）
- kind: `component`
- sourceLayer: `local`
- status: `approved`
- AI 使用資格: `allowed`（allowed=可正式使用；reference-only=只供理解/參考；blocked=不得使用）
- aliases: —
- Figma: 04_功能元件 / Feature Components · 分頁 3 / [prediction-card-768-variant2] prediction-card_768/Variant2 / prediction-card_768 Variant2 / `190:237483` / https://www.figma.com/design/UX9EY190SGnpNiowHlwiUB?node-id=190-237483
- Figma 原生類型: `COMPONENT`; Library Key=available; publishStatus=`NOT_CHECKED（安全匯出，以 Figma Library 面板為準）`
- Library Key: component=`a648382329adf58ef2620c43574657a9cf70a76c`; componentSet=`—`
- Component Properties: —（嚴格唯讀匯出僅由 Variant 名稱推導；TEXT／BOOLEAN／INSTANCE_SWAP 請由 MCP 即時讀取）
- 功能說明: 作為圖片預覽內容的 prediction card 尺寸／版型子元件，用於預覽畫布內呈現事件卡片。
- 遷移備註: 有大量預覽使用，但為單一 COMPONENT 子版型；需人工確認是否應併入 prediction-card Component Set 後再遷移
- Page Type: List
...
- Variant Key: unknown|unknown|default
- States: default
- Components: prediction-card-768-variant2
- Variants: State / Size / Type / Layout / Theme / Density / Orientation / Emphasis
- Theme: explicit=unknown, strategy=variables, support=variables
- Review: reviewed / confidence=0.86
- Original Review Action: candidate
- Evidence: migrationScore=70；usageEvidence：directInstances=0、screens=32、components=1，method=figma-main-component；屬 preview 內部 prediction-card 子元件，並非 Component Set
- Legacy mapping: —
- Migration Score/Source: — / 資料庫管理 - 事件參數管理 / 參數類別管理 / 事件主類別管理 / 事件次類別管理 /         Key Screens 🔖v0.1.0-20260313-當前版本 / 6121:44846
- AI usage rules: 優先使用 internal-core，再 vendor-core，再 shared-pattern/feature；不得自行重建 Vendor Master。
...
## 價格層（通用候選） / Price Tier Generic Candidate
- canonicalId: `price-tier-generic-candidate`
- canonicalFamilyId: `price-tier-generic-candidate`（同一 family 的其他 RWD/Theme variants可用此值查找）
- kind: `component`
- sourceLayer: `candidate`
- status: `approved`
- AI 使用資格: `allowed`（allowed=可正式使用；reference-only=只供理解/參考；blocked=不得使用）
- aliases: —
- Figma: 04_功能元件 / Feature Components · 分頁 3 / [price-tier-generic-candidate] 價格層（通用候選） / Price Tier Generic Candidate / `144:6573` / https://www.figma.com/design/UX9EY190SGnpNiowHlwiUB?node-id=144-6573
- Figma 原生類型: `COMPONENT_SET`; Library Key=available; publishStatus=`NOT_CHECKED（安全匯出，以 Figma Library 面板為準）`
- Library Key: component=`—`; componentSet=`e9fbbcd4b3e4f3f1125fed3c64bcf5d9b020744a`
- Component Properties: 價格#7695:0 (TEXT)；價格層級 (VARIANT: 0.00~0.09/0.10~0.19/0.20~0.29/0.30~0.39/0.40~0.49/0.50~0.59/0.60~0.69/0.70~0.79/0.80~0.89/0.90~1.00/新增價格)（嚴格唯讀匯出僅由 Variant 名稱推導；TEXT／BOOLEAN／INSTANCE_SWAP 請由 MCP 即時讀取）
- 功能說明: 提供通用價格層級與新增價格狀態，可能作為其他價格層元件的早期版本或共用基底。
- 遷移備註: 不能只因使用量為 0 直接否定，但目前與已使用的價格層元件功能重疊，需確認是否為舊版、未接線版本或可合併的通用基底。
- Page Type: Other
- Responsive Variants: unknown
- Variant Key: unknown|unknown|default
- States: default
- Components: price-tier-generic-candidate
- Variants: State / Size / Type / Layout / Theme / Density / Orientation / Emphasis
- Theme: explicit=unknown, strategy=manual, support=unknown
- Review: reviewed / confidence=0.78
- Original Review Action: —
- Evidence: migrationScore=50，usageEvidence 為 figma-main-component，但 directInstances=0、usedByScreenCount=0、usedByComponentCount=0。；仍為正式 Component Set，具 11 個價格層級 variants，包含 0.00~1.00 與新增價格。；同批已有使用量明確的 Desktop 價格層與 Mobile 價格層結構，本元件目前沒有可驗證的產品畫面引用。
- Legacy mapping: —
- Migration Score/Source: — / 事件設置 - 事件管理列表 / 新增事件&匯入議題 / 第三方事件審核列表 / 事件參數管理 / 單一事件結果 / 多時間軸事件 / 庫存管理 /         庫存管理🔖v0.1.1-20260731 / 7712:452574
- AI usage rules: 優先使用 internal-core，再 vendor-core，再 shared-pattern/feature；不得自行重建 Vendor Master。
...
## 損益數據提示 / Profit/Loss Metric Tooltip
- canonicalId: `profit-loss-metric-tooltip`
- canonicalFamilyId: `profit-loss-metric-tooltip`（同一 family 的其他 RWD/Theme variants可用此值查找）
- kind: `component`
- sourceLayer: `candidate`
- status: `approved`
- AI 使用資格: `allowed`（allowed=可正式使用；reference-only=只供理解/參考；blocked=不得使用）
- aliases: —
- Figma: 04_功能元件 / Feature Components · 分頁 3 / [profit-loss-metric-tooltip] 損益數據提示 / Profit/Loss Metric Tooltip / `155:347400` / https://www.figma.com/design/UX9EY190SGnpNiowHlwiUB?node-id=155-347400
- Figma 原生類型: `COMPONENT_SET`; Library Key=available; publishStatus=`NOT_CHECKED（安全匯出，以 Figma Library 面板為準）`
- Library Key: component=`—`; componentSet=`e2d2fbf426106a0a00f1ab43e9f447edb4567b36`
- Component Properties: 項目 (VARIANT: 總投分/交易筆數/結算費/手續費/系統營收/投分量/玩家賠付/投入成本)（嚴格唯讀匯出僅由 Variant 名稱推導；TEXT／BOOLEAN／INSTANCE_SWAP 請由 MCP 即時讀取）
- 功能說明: 顯示事件損益與投分歷史各項統計指標的說明 Tooltip，包括總投分、交易筆數、費用、營收與投入成本。
- 遷移備註: 指標 Tooltip variants 完整但尚未接入正式畫面；保留 Light 結構作候選，並建議確認是否要以 variables 合併 Theme。
- Page Type: List
- Responsive Variants: unknown
- Variant Key: unknown|unknown|default
- States: default
- Components: profit-loss-metric-tooltip
- Variants: State / Size / Type / Layout / Theme / Density / Orientation / Emphasis
- Theme: explicit=unknown, strategy=manual, support=unknown
- Review: reviewed / confidence=0.82
- Original Review Action: —
- Evidence: usageEvidence 為 figma-main-component，但 directInstanceCount、nestedDependencyCount、usedByScreenCount、usedByComponentCount 均為 0。；Component Set 仍具有 8 個完整指標 variants：總投分、交易筆數、投分量、玩家賠付、手續費、結算費、系統營收、投入成本；Light/Dark 各存在一套相同結構。
- Legacy mapping: —
- Migration Score/Source: — / 報表紀錄 - 事件損益查詢 / 投分歷史紀錄 /         Key Screens 🔖v0.1.1-20260717(新增補扣單) / 10068:220063
- AI usage rules: 優先使用 internal-core，再 vendor-core，再 shared-pattern/feature；不得自行重建 Vendor Master。
...
## Rush Hour 投分參數設定 / Rush Hour Wager Parameter Settings
- canonicalId: `rush-hour-wager-parameter-settings`
- canonicalFamilyId: `rush-hour-wager-parameter-settings`（同一 family 的其他 RWD/Theme variants可用此值查找）
- kind: `component`
- sourceLayer: `candidate`
- status: `approved`
- AI 使用資格: `allowed`（allowed=可正式使用；reference-only=只供理解/參考；blocked=不得使用）
- aliases: —
- Figma: 04_功能元件 / Feature Components · 分頁 3 / [rush-hour-wager-parameter-settings] Rush Hour 投分參數設定 / Rush Hour Wager Parameter Settings / `121:552870` / https://www.figma.com/design/UX9EY190SGnpNiowHlwiUB?node-id=121-552870
- Figma 原生類型: `COMPONENT_SET`; Library Key=available; publishStatus=`NOT_CHECKED（安全匯出，以 Figma Library 面板為準）`
- Library Key: component=`—`; componentSet=`7fe9022fcf6bad13df2451874788c1279ba6ee1e`
- Component Properties: status (VARIANT: enable/enable--filled in)；device (VARIANT: PC/tablet/Mobile)（嚴格唯讀匯出僅由 Variant 名稱推導；TEXT／BOOLEAN／INSTANCE_SWAP 請由 MCP 即時讀取）
- 功能說明: 提供 Rush Hour 專用投分參數編輯、回復預設值與儲存操作。
- 遷移備註: variants 完整，需人工確認
- Page Type: Edit
- Responsive Variants: unknown
- Variant Key: unknown|unknown|default
- States: default
- Components: rush-hour-wager-parameter-settings
- Variants: State / Size / Type / Layout / Theme / Density / Orientation / Emphasis
- Theme: explicit=unknown, strategy=variables, support=variables
- Review: reviewed / confidence=0.76
- Original Review Action: —
- Evidence: usage=0/0; v=6
- Legacy mapping: —
- Migration Score/Source: — / 事件設置 - 事件管理列表 / 新增事件&匯入議題 / 第三方事件審核列表 / 事件參數管理 / 單一事件結果 / 多時間軸事件 / 庫存管理 /         Key Screens 🔖v0.1.1-20260529 (避險前先做) / 7243:425962
- AI usage rules: 優先使用 internal-core，再 vendor-core，再 shared-pattern/feature；不得自行重建 Vendor Master。
...
## 查看事件次類別對話框 / View Event Subcategories Dialog
- canonicalId: `event-main-category-subcategory-viewer`
- canonicalFamilyId: `event-main-category-subcategory-viewer`（同一 family 的其他 RWD/Theme variants可用此值查找）
- kind: `component`
- sourceLayer: `candidate`
- status: `approved`
- AI 使用資格: `allowed`（allowed=可正式使用；reference-only=只供理解/參考；blocked=不得使用）
- aliases: —
- Figma: 04_功能元件 / Feature Components · 分頁 5 / [event-main-category-subcategory-viewer] 查看事件次類別對話框 / View Event Subcategories Dialog / `198:276195` / https://www.figma.com/design/UX9EY190SGnpNiowHlwiUB?node-id=198-276195
- Figma 原生類型: `COMPONENT_SET`; Library Key=available; publishStatus=`NOT_CHECKED（安全匯出，以 Figma Library 面板為準）`
- Library Key: component=`—`; componentSet=`40b8b464f3e1dbe3b77700fa6b9123a0d1ac9931`
- Component Properties: Property 1 (VARIANT: PC&Tablet/Mobile)（嚴格唯讀匯出僅由 Variant 名稱推導；TEXT／BOOLEAN／INSTANCE_SWAP 請由 MCP 即時讀取）
- 功能說明: 封裝事件主類別管理中的「查看已掛載次類別」對話框，依 PC&Tablet／Mobile 切換版型並組合標題、清單與操作。
- 遷移備註: 具有完整的查看次類別 RWD 對話框結構，但目前未被正式產品畫面引用，先保留為候選。
- Page Type: Create
- Responsive Variants: unknown
- Variant Key: unknown|unknown|default
- States: default
- Components: event-main-category-subcategory-viewer
- Variants: State / Size / Type / Layout / Theme / Density / Orientation / Emphasis
- Theme: explicit=unknown, strategy=manual, support=unknown
- Review: reviewed / confidence=0.84
- Original Review Action: —
- Evidence: migrationScore=50，usageEvidence：directInstances=0、nestedDependencies=0、screens=0、components=0，目前沒有產品畫面使用證據；Component Set 具有 PC&Tablet 與 Mobile 兩個 variants，具明確 RWD 封裝；composition 內含標題、列表與操作內容，結構完整
- Legacy mapping: —
- Migration Score/Source: — / 資料庫管理 - 事件參數管理 / 參數類別管理 / 事件主類別管理 / 事件次類別管理 /         Key Screens 🔖v0.1.1-20260423 / 6154:558626
- AI usage rules: 優先使用 internal-core，再 vendor-core，再 shared-pattern/feature；不得自行重建 Vendor Master。
...
## 掛載事件次類別選擇器 / Event Subcategory Selector
- canonicalId: `event-subcategory-selector`
- canonicalFamilyId: `event-subcategory-selector`（同一 family 的其他 RWD/Theme variants可用此值查找）
- kind: `component`
- sourceLayer: `candidate`
- status: `approved`
- AI 使用資格: `allowed`（allowed=可正式使用；reference-only=只供理解/參考；blocked=不得使用）
- aliases: —
- Figma: 04_功能元件 / Feature Components · 分頁 5 / [event-subcategory-selector] 掛載事件次類別選擇器 / Event Subcategory Selector / `198:276180` / https://www.figma.com/design/UX9EY190SGnpNiowHlwiUB?node-id=198-276180
- Figma 原生類型: `COMPONENT_SET`; Library Key=available; publishStatus=`NOT_CHECKED（安全匯出，以 Figma Library 面板為準）`
- Library Key: component=`—`; componentSet=`75da0fb8ac77f79cbd56097fd5b1f19eb6b6cf49`
- Component Properties: type (VARIANT: empty/full)；status (VARIANT: active)；device (VARIANT: Mobile/PC&Tablet)（嚴格唯讀匯出僅由 Variant 名稱推導；TEXT／BOOLEAN／INSTANCE_SWAP 請由 MCP 即時讀取）
- 功能說明: 在新增／編輯主類別第 2 步封裝事件次類別選擇內容，支援空／滿資料以及 Mobile、PC&Tablet 版型。
- 遷移備註: 目前沒有實際使用證據，但具完整流程語意與 RWD variants；保留為候選，待確認是否要以此 wrapper 取代現有直接組裝的 List Group。
- Page Type: Create
- Responsive Variants: unknown
- Variant Key: unknown|unknown|default
- States: default
- Components: event-subcategory-selector
- Variants: State / Size / Type / Layout / Theme / Density / Orientation / Emphasis
- Theme: explicit=unknown, strategy=manual, support=unknown
- Review: reviewed / confidence=0.83
- Original Review Action: —
- Evidence: inventory migrationScore=50，usageEvidence 為 directInstances=0、screens=0，表示目前沒有可驗證的產品 instance；Component Set 仍具有正式流程語意與 4 個 child variants，包含 type=empty/full、device=Mobile/PC&Tablet；parentPath 位於新增／編輯主類別 step.2 的 Light 功能規格區，且不是單純泛用 Master
- Legacy mapping: —
- Migration Score/Source: — / 資料庫管理 - 事件參數管理 / 參數類別管理 / 事件主類別管理 / 事件次類別管理 /         Key Screens 🔖v0.1.1-20260423 / 6082:28923
- AI usage rules: 優先使用 internal-core，再 vendor-core，再 shared-pattern/feature；不得自行重建 Vendor Master。
